...
const emojiService_1 = require("./emojiService");
class MessageService {
    constructor() {
        this.db = (0, database_1.getDatabase)();
        this.emojiService = new emojiService_1.EmojiService();
    }
    async createMessage(messageData) {
        console.log('Criando mensagem no serviço:', messageData);
        const validation = Message_1.Message.validateCreate(messageData);
        ...
        await this.db.run(`INSERT INTO messages (id, content, type, roomId, senderId, replyToId, isEdited, isDeleted, status, createdAt, updatedAt)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`, [
    ...
    async getMessageById(id) {
        ...
        return messageData ? new Message_1.Message(messageData) : null;
    }
    async getMessagesByRoom(roomId, limit = 50, offset = 0) {
        console.log('Buscando mensagens da sala:', { roomId, limit, offset });
        const messages = await this.db.all(`SELECT m.*, u.username, u.nickname, u.avatar
       FROM messages m
       ...
       LIMIT ? OFFSET ?`, [roomId, limit, offset]);
        ...
        const messagesWithReplies = await Promise.all(messages.map(async (msg, index) => {
            ...
            let replyTo = null;
            if (msg.replyToId) {
                const replyData = await this.db.get(`SELECT m.id, m.content, u.username
             FROM messages m
             JOIN users u ON m.senderId = u.id
    ...
    async getMessageWithSender(id) {
        ...
        const messageData = await this.db.get(`SELECT m.*, u.username, u.nickname, u.avatar
       FROM messages m
       JOIN users u ON m.senderId = u.id
       WHERE m.id = ?`, [id]);
        ...
        if (messageData.replyToId) {
         ...
         FROM messages m
         JOIN users u ON m.senderId = u.id
         WHERE m.id = ?`, [messageData.replyToId]);
            if (replyData) {
                replyTo = {
                    id: replyData.id,
                    content: replyData.content,
    ...
    async updateMessage(id, updateData, userId) {
        ...
        const validation = Message_1.Message.validateUpdate(updateData);
        if (!validation.isValid) {
            throw new Error(`Dados inválidos: ${validation.errors.join(', ')}`);
        }
        const updateFields = [];
    ...
    async editMessage(id, newContent, userId) {
    ...
    async deleteMessage(id, userId) {
        ...
        const result = await this.db.run('UPDATE messages SET isDeleted = 1, updatedAt = ? WHERE id = ?', [new Date().toISOString(), id]);
        return result.changes > 0;
    }
    async searchMessages(query, roomId, limit = 20) {
       ...
       ORDER BY m.createdAt DESC
       LIMIT ?`, [roomId, searchQuery, limit]);
    ...
    async getMessageCount(roomId) {
        const result = await this.db.get('SELECT COUNT(*) as count FROM messages WHERE roomId = ? AND isDeleted = 0', [roomId]);
    ...
    async getMessagesByUser(userId, limit = 50) {
        const messages = await this.db.all(`SELECT m.*, u.username, u.nickname, u.avatar
       FROM messages m
       JOIN users u ON m.senderId = u.id
    ...
    async getUnreadMessageCount(roomId, lastReadTimestamp) {
        const result = await this.db.get('SELECT COUNT(*) as count FROM messages WHERE roomId = ? AND isDeleted = 0 AND createdAt > ?', [roomId, lastReadTimestamp.toISOString()]);
        return result?.count || 0;
    }
    ...
    async markRoomAsRead(roomId, userId) {
        try {
            ...
            const result = await this.db.run(`INSERT OR REPLACE INTO user_room_reads (userId, roomId, lastReadAt, updatedAt) 
         VALUES (?, ?, ?, ?)`, [userId, roomId, new Date().toISOString(), new Date().toISOString()]);
            return result.changes > 0;
        }
        catch (error) {
    ...
    async getLastReadTimestamp(roomId, userId) {
        try {
            const result = await this.db.get('SELECT lastReadAt FROM user_room_reads WHERE roomId = ? AND userId = ?', [roomId, userId]);
    ...
    async markMessageAsDelivered(messageId) {
        try {
            const result = await this.db.run('UPDATE messages SET status = ?, updatedAt = ? WHERE id = ? AND status = ?', ['delivered', new Date().toISOString(), messageId, 'sent']);
            return result.changes > 0;
        }
        catch (error) {
    ...
    async markMessageAsRead(messageId) {
        try {
            const result = await this.db.run('UPDATE messages SET status = ?, updatedAt = ? WHERE id = ? AND status IN (?, ?)', ['read', new Date().toISOString(), messageId, 'sent', 'delivered']);
            return result.changes > 0;
        }
        catch (error) {
    ...
    async markAllMessagesAsRead(roomId, userId) {
        try {
            ...
            const messageCount = await this.db.get('SELECT COUNT(*) as count FROM messages WHERE roomId = ?', [roomId]);
            console.log('Total de mensagens na sala:', messageCount?.count || 0);
            // Verificar mensagens com status específicos
            const statusCount = await this.db.get('SELECT COUNT(*) as count FROM messages WHERE roomId = ? AND senderId != ? AND status IN (?, ?)', [roomId, userId, 'sent', 'delivered']);
            console.log('Mensagens que podem ser marcadas como lidas:', statusCount?.count || 0);
            // Executar a atualização
            const result = await this.db.run(`UPDATE messages 
         SET status = ?, updatedAt = ? 
         WHERE roomId = ? AND senderId != ? AND status IN (?, ?)`, ['read', new Date().toISOString(), roomId, userId, 'sent', 'delivered']);
