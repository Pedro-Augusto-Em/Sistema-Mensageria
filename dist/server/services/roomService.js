...
class RoomService {
    ...
    async createRoom(roomData) {
        const validation = Room_1.Room.validateCreate(roomData);
        ...
        await this.db.run(`INSERT INTO rooms (id, name, description, type, avatar, createdBy, members, admins, isPrivate, createdAt, updatedAt)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`, [
    ...
    async getRoomById(id) {
        ...
        return new Room_1.Room(room);
    }
    async getRoomWithMembers(id) {
        const roomData = await this.db.get('SELECT * FROM rooms WHERE id = ?', [id]);
        if (!roomData)
            return null;
        ...
        const memberUsers = await Promise.all(members.map(async (memberId) => {
            const userData = await this.db.get('SELECT id, username, nickname, avatar, status FROM users WHERE id = ?', [memberId]);
            return userData ? {
                id: userData.id,
    ...
    async getRoomsByUser(userId) {
        const rooms = await this.db.all('SELECT * FROM rooms WHERE json_extract(members, "$") LIKE ? ORDER BY updatedAt DESC', [`%${userId}%`]);
        return rooms.map(room => {
            ...
            return new Room_1.Room(roomData);
        });
    }
    async getDirectMessageRoom(userId1, userId2) {
        const rooms = await this.db.all(`SELECT * FROM rooms 
       WHERE type = 'direct' 
       AND json_extract(members, "$") LIKE ? 
       AND json_extract(members, "$") LIKE ?`, [`%${userId1}%`, `%${userId2}%`]);
        if (rooms.length === 0)
            return null;
        const room = rooms[0];
        const roomData = {
    ...
    async createDirectMessage(userId1, userId2) {
    ...
    async updateRoom(id, updateData, userId) {
        ...
        const validation = Room_1.Room.validateUpdate(updateData);
        if (!validation.isValid) {
            throw new Error(`Dados inválidos: ${validation.errors.join(', ')}`);
        }
        const updateFields = [];
    ...
    async addMember(roomId, userId, adminUserId) {
        ...
        await this.db.run('UPDATE rooms SET members = ?, updatedAt = ? WHERE id = ?', [JSON.stringify(newMembers), new Date().toISOString(), roomId]);
        return true;
    }
    async removeMember(roomId, userId, adminUserId) {
        const room = await this.getRoomById(roomId);
        if (!room) {
            throw new Error('Sala não encontrada');
        }
        ...
        await this.db.run('UPDATE rooms SET members = ?, admins = ?, updatedAt = ? WHERE id = ?', [JSON.stringify(newMembers), JSON.stringify(newAdmins), new Date().toISOString(), roomId]);
        return true;
    }
    async addAdmin(roomId, userId, adminUserId) {
        const room = await this.getRoomById(roomId);
        if (!room) {
            throw new Error('Sala não encontrada');
        }
        ...
        await this.db.run('UPDATE rooms SET admins = ?, updatedAt = ? WHERE id = ?', [JSON.stringify(newAdmins), new Date().toISOString(), roomId]);
        return true;
    }
    async removeAdmin(roomId, userId, adminUserId) {
        const room = await this.getRoomById(roomId);
        if (!room) {
            throw new Error('Sala não encontrada');
        }
    ...
    async searchRooms(query, userId) {
       ...
       AND json_extract(members, "$") LIKE ?
       AND isPrivate = 0
       ORDER BY name`, [searchQuery, searchQuery, `%${userId}%`]);
        return rooms.map(room => {
            const roomData = {
                ...room,
                members: JSON.parse(room.members),
    ...
    async getPublicRooms() {
        const rooms = await this.db.all('SELECT * FROM rooms WHERE isPrivate = 0 ORDER BY name');
        return rooms.map(room => {
            const roomData = {
                ...room,
                members: JSON.parse(room.members),
