...
router.get('/', authenticateToken, asyncHandler(async (req: Request, res: Response) => {
  ...
  const roomsWithLastMessage = await Promise.all(
    roomsWithMembers.map(async (room) => {
      try {
        // Buscar última mensagem
        const messageService = new (await import('../services/messageService')).MessageService();
...
router.post('/:id/members', authenticateToken, asyncHandler(async (req: Request, res: Response) => {
  ...
  if (success) {
    ...
    if (io) {
      ...
      const userSockets = (req as any).app.get('userSockets') || new Map();
